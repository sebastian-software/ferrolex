...
| `hu_HU` | Latin, agglutinative | AFF UTF-8 with ISO-8859-2 fallback; DIC UTF-8 | Lenient fixture | The reviewed decoder handles isolated legacy AFF bytes and NEL separators without rewriting source bytes. The runtime-cache roundtrip covers stored, affixed, and compound forms, but strict import deliberately rejects the constructs listed in the [Hungarian compatibility boundary](compatibility-fixtures.md#hungarian-compatibility-boundary). |
