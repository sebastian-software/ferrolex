...
| `hu_HU` | Latin, agglutinative | AFF UTF-8 with ISO-8859-2 fallback; DIC UTF-8 | Lenient fixture | The reviewed decoder handles isolated legacy AFF bytes and NEL separators without rewriting source bytes. Stored, affixed, and compound probes survive the runtime-cache roundtrip; the exact strict-import error set is pinned and rechecked in the [Hungarian compatibility boundary](compatibility-fixtures.md#hungarian-compatibility-boundary). |
