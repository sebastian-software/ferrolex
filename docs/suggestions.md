...
after `max_word_scalars + 2 * max_edit_distance` scalar values. Sources without
morphological expansion skip seed traversal entirely. Thus an incomplete result
remains explicit through `Completeness`.
