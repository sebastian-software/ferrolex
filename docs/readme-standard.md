...
The root README uses native mdtheme composition from `README.md.src`, with
Sebastian outside and Ferramenta inside. See [README themes](readme-theme.md). Its
...
The repository root uses the native shared frames for the project page. The eight
...
The root is generated by mdtheme. `scripts/mirror-readme-footer.py` copies its
outer Sebastian footer into the crate README marker sections. Standards
validates ownership and leaves the root README untouched.
...
generator from a pinned commit and writes or verifies the subpackage family blocks.

```sh
mise run readme:write                     # regenerate the project README
scripts/generate-readme-family.sh          # regenerate package family blocks
